  ...
  return (
    <div className="min-h-screen pt-24">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {post.coverImage && (
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl overflow-hidden mb-8 h-64 sm:h-80">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          {post.tags?.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {post.tags.map(tag => <span key={tag} className="flex items-center gap-1 tag"><Tag size={10} /> {tag}</span>)}
            </div>
          )}
          <h1 className="text-3xl sm:text-4xl font-display font-bold text-white leading-tight mb-4">{post.title}</h1>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
            ...
            code: ({ inline, children }) => inline
              ? <code className="px-1.5 py-0.5 rounded text-sm text-indigo-300 font-mono" style={{ background: 'rgba(99,102,241,0.15)' }}>{children}</code>
              : <code className="block p-4 rounded-xl text-sm font-mono text-slate-200 overflow-x-auto mb-4" style={{ background: 'rgba(13,20,36,0.9)', border: '1px solid rgba(99,102,241,0.15)' }}>{children}</code>,
            blockquote: ({ children }) => <blockquote className="border-l-4 border-indigo-500 pl-4 my-4 text-slate-400 italic">{children}</blockquote>,
            ul: ({ children }) => <ul className="list-none space-y-2 mb-4">{children}</ul>,
            li: ({ children }) => <li className="flex items-start gap-2 text-slate-300"><span className="text-indigo-400 mt-1 flex-shrink-0">▸</span>{children}</li>,
            img: ({ src, alt }) => <img src={src} alt={alt} className="rounded-xl w-full my-6" />,
        ...
        <div className="mt-12 pt-8 border-t border-white/5 flex flex-col sm:flex-row gap-4 items-center justify-between">
          ...
          <Link to="/#contact" className="btn-primary text-sm"><span>Get in Touch →</span></Link>
        </div>
      </div>
    </div>
  )
